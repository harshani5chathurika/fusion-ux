...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: member } = await supabase
      ...
      .select("organization_id, organizations(id, ai_credits, plan)")
      .eq("user_id", user.id)
      .single();
    ...
    const credits: number = org?.ai_credits ?? 0;
    const plan: string = org?.plan ?? "free";

    if (credits <= 0 && plan !== "pro") {
      return NextResponse.json({ error: "no_credits", credits: 0 }, { status: 402 });
    }

    const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });

    // Step 1: GPT-4o analyzes the violation and generates fix spec + DALL-E prompt
...
Return a JSON object with these exact keys:
{
  ...
  "figma_prompt": "A Figma AI / FigJam prompt a designer can paste directly. Include component names, variant properties, spacing values, and color tokens."
    ...
    // Step 2: Generate the fixed UI mockup with DALL-E 3
    ...
    // Step 5: Deduct 1 credit (skip for pro)
    if (plan !== "pro") {
      await supabase
        .from("organizations")
    ...
    return NextResponse.json({
      ...
      credits_remaining: plan === "pro" ? null : credits - 1,
    });
  } catch (e) {
