    ...
    try {
      ...
      if (projectMode === "new" && newProjectName.trim()) {
        const { data: newProject, error: projectError } = await supabase
          ...
          .select()
          .single();
        if (projectError || !newProject) throw new Error(projectError?.message ?? "Failed to create project");
      ...
      if (auditError || !audit) throw new Error(auditError?.message ?? "Failed to create audit");
      ...
      if (auditType === "figma" && data.target_url) {
        const figmaRes = await fetch("/api/figma", {
          ...
          body: JSON.stringify({ figma_url: data.target_url, figma_token: figmaToken, audit_id: audit.id, user_id: user.id }),
        });
        if (!figmaRes.ok) {
      ...
      if (uploadedFiles.length > 0) {
        for (const [idx, uploadedFile] of Array.from(uploadedFiles.entries())) {
          ...
          if (uploadError) throw new Error(`Failed to upload screenshot "${uploadedFile.file.name}": ${uploadError.message}`);
          const { data: { publicUrl } } = supabase.storage.from("audit-screenshots").getPublicUrl(path);
          uploadedUrls.push(publicUrl);
          await supabase.from("audit_screenshots").insert({
            audit_id: audit.id, url: publicUrl, filename: uploadedFile.file.name,
            file_size: uploadedFile.file.size, order_index: idx, annotations: [],
          });
        }
      }
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold">Assign to Project</h2>
        ...
        <div className="flex gap-2">
          {[
            ...
            { key: "existing", label: "Existing project" },
            { key: "new", label: "New project" },
          ].map((opt) => (
            <button
              key={opt.key}
              type="button"
              onClick={() => { setProjectMode(opt.key as typeof projectMode); setSelectedProjectId(null); }}
              className={cn(
        ...
        {projectMode === "existing" && (
          <div className="space-y-2">
            {projects.length === 0 ? (
              <p className="text-xs text-muted-foreground italic">No projects yet. Create a new one instead.</p>
            ) : (
              <div className="relative">
                <button
                  type="button"
                  onClick={() => setProjectDropdownOpen((v) => !v)}
                  className="w-full flex items-center justify-between px-3 py-2.5 rounded-lg border border-input bg-background text-sm hover:border-primary/50 transition-colors"
                >
                  <div className="flex items-center gap-2">
                    <FolderOpen className="h-4 w-4 text-violet-500" />
                ...
                {projectDropdownOpen && (
                  <>
                    ...
                    <div className="absolute z-20 top-full mt-1 w-full bg-background border border-border rounded-xl shadow-lg overflow-hidden max-h-52 overflow-y-auto">
                      {projects.map((p) => (
                        <button
                          key={p.id}
                          type="button"
                        ...
                        >
                          ...
                          {selectedProjectId === p.id && <Check className="h-4 w-4 text-primary flex-shrink-0" />}
                        </button>
                      ))}
                    </div>
                  </>
        ...
        {projectMode === "new" && (
          <div className="space-y-3 p-3 rounded-lg bg-muted/30 border border-border">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-semibold text-violet-700 dark:text-violet-300">New project</span>
            </div>
            <div className="space-y-1">
              <label className="text-xs font-medium">Project name *</label>
              <input
                type="text"
                placeholder="e.g. SriLankan Airlines Website"
      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[
            { type: "image" as AuditType, icon: ImageIcon, label: "Screenshots", desc: "Upload UI screenshots for AI visual analysis" },
            { type: "url" as AuditType, icon: Globe, label: "Website URL", desc: "Analyze a live website — auto screenshots" },
            { type: "figma" as AuditType, icon: Figma, label: "Figma Design", desc: "Audit designs before development" },
            { type: "full" as AuditType, icon: FileSearch, label: "Full Audit", desc: "Screenshots + URL for comprehensive coverage" },
          ].map((option) => (
            <button
              key={option.type}
              type="button"
              ...
              className={cn(
                ...
                auditType === option.type ? "border-primary bg-primary/5" : "border-border hover:border-primary/50 hover:bg-accent"
              )}
            >
              <div className="flex items-center justify-between">
                <option.icon className={cn("h-5 w-5", auditType === option.type ? "text-primary" : "text-muted-foreground")} />
                {auditType === option.type && <Check className="h-4 w-4 text-primary" />}
              </div>
              <div>
                <p className={cn("text-sm font-semibold", auditType === option.type ? "text-primary" : "")}>{option.label}</p>
      ...
      <div className="rounded-xl border border-border bg-card p-5 space-y-4">
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            className={cn(
              "w-full px-3 py-2.5 rounded-lg border bg-background text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring",
        ...
        <div className="space-y-2">
          ...
          <textarea
            ...
            className="w-full px-3 py-2.5 rounded-lg border border-input bg-background text-sm resize-none placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring"
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className={cn(
                  "w-full pl-9 pr-4 py-2.5 rounded-lg border bg-background text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring",
        ...
        {auditType === "figma" && (
          <>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">Get your token at figma.com → Account Settings → Personal Access Tokens</p>
            </div>
          </>
        )}
          ...
          <div className="flex gap-2">
            {["desktop", "mobile", "tablet"].map((device) => (
              <label key={device} className="flex-1">
                <input type="radio" value={device} className="sr-only" {...register("device_type")} />
                <div className="px-3 py-2 rounded-lg border text-sm text-center cursor-pointer capitalize transition-all hover:border-primary/50">
                  {device}
                </div>
              </label>
            ))}
      ...
      {(auditType === "image" || auditType === "full") && (
        <div className="rounded-xl border border-border bg-card p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground">{uploadedFiles.length}/5 files</span>
          </div>
          <div
            {...getRootProps()}
            className={cn(
          ...
          >
            ...
            <p className="text-sm font-medium">{isDragActive ? "Drop screenshots here…" : "Drag & drop screenshots here"}</p>
            <p className="text-xs text-muted-foreground mt-1">or click to select files · PNG, JPG, WebP · max 10MB each · up to 5 files</p>
          </div>
            ...
            <div className="grid grid-cols-5 gap-2">
              {uploadedFiles.map((file, idx) => (
                <div key={idx} className="relative group rounded-lg overflow-hidden border border-border">
                  ...
                  <img src={file.preview} alt={file.file.name} className="w-full h-16 object-cover" />
                  <button
                    type="button"
      ...
      <div className="rounded-xl border border-violet-200 dark:border-violet-800 bg-violet-50 dark:bg-violet-950/30 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-violet-800 dark:text-violet-200">AI Analysis will run automatically</p>
            <p className="text-xs text-violet-600 dark:text-violet-300 mt-1">
              Claude + Gemini will evaluate your UI against all 153 checks across 12 heuristic categories.
      ...
      >
        {isSubmitting ? (
          <><Loader2 className="h-4 w-4 animate-spin" />Creating audit & running AI analysis…</>
        ) : (
